...
class ProfileEditViewController02: UIViewController,UITextFieldDelegate,UIScrollViewDelegate,UIPickerViewDelegate,UIPickerViewDataSource,UINavigationControllerDelegate,UIImagePickerControllerDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        jobList = jobCategoryList + jobTypeList
    ...
    @IBAction func save(_ sender: Any) {
        ...
        if let job = userInfo.favoriteJob{
                udSetting.save(key: .job, value: job)
        }
        if let appeal = userInfo.selfAppeal{
                udSetting.save(key: .appeal, value: appeal)
        }
    ...
    func initialSet(){
        ...
        userInfo.selfAppeal = udSetting.read(key: .appeal)
        ...
        appealTextField.text = userInfo.selfAppeal
    ...
    func createTag(){
        ...
        for jobTag in favoriteJob{
            if jobCategoryList.contains(jobTag){
                let tagNum = jobCategoryList.index(of: jobTag)!
                print("357tag:\(tagNum)")
                addTag(senderTag: tagNum, sectionType: .industry)
            }
            if jobTypeList.contains(jobTag){
                let tagNum = jobTypeList.index(of: jobTag)! + jobCategoryList.count
    ...
    func removeTag(senderTag:Int,senderOrder:Int,sectionType:jobTagType){
        print("425/order:\(senderOrder)")
        print("426/tag:\(senderTag)")
...
extension ProfileEditViewController02{
